...
# originally coded to work with 1-12 car datasets.
...
prepare_one_lot=parseargs.prepareonelot

print(prepare_one_lot)


ONELOT_OUTPUT_DIR=os.path.join(CURRENT_DIR,"common_json_output")
...
c.execute("SELECT * from homepage_save_progress where username=?",(USERNAME,))
results = c.fetchall()
results = sorted(results,key=lambda x: x[2])

# if one lot, then prepare the output dir only once
if prepare_one_lot:
    prepare_output_dir(ONELOT_OUTPUT_DIR)
...
for result in results:
    ...
    if prepare_one_lot:
        conv_output_dir = ONELOT_OUTPUT_DIR
    else:    
